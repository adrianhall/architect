...
1. **`remove_node` reverse produces `batch`** — Create a `remove_node` with 2 connected edges. Verify the reverse is a `batch` with `add_node` + 2 × `add_edge`.

1. **`remove_node` with no connected edges** — Verify the reverse `batch` contains only `add_node`.

1. **`move_node` reverse swaps `from` and `to`** — Verify `from` becomes `to` and vice versa, `nodeId` is preserved.

1. **`add_edge` reverse produces `remove_edge`** — Verify the edge object is preserved.

1. **`remove_edge` reverse produces `add_edge`** — Verify the edge object is preserved.

1. **`update_node_data` reverse swaps `from` and `to`** — Verify `nodeId` preserved, data swapped.

1. **`update_edge_data` reverse swaps `from` and `to`** — Verify `edgeId` preserved, data swapped.

1. **`batch` reverse reverses order and reverses each sub-operation** — Create a batch with 3 operations. Verify the reverse has them in reversed order, each individually reversed.

### `applyOperation` Tests

1. **`add_node` adds node to array** — Start with 0 nodes, apply `add_node`, verify 1 node.

1. **`remove_node` removes node and connected edges** — Start with 2 nodes + 1 edge. Apply `remove_node` for one node (listing the edge as connected). Verify 1 node, 0 edges remain.

1. **`remove_node` preserves unrelated edges** — Verify edges not listed in `connectedEdges` are kept.

1. **`move_node` updates position** — Apply `move_node` with `to: { x: 100, y: 200 }`. Verify node position changed.

1. **`move_node` preserves other nodes** — Verify non-targeted nodes keep their positions.

1. **`add_edge` adds edge to array** — Start with 0 edges, apply `add_edge`, verify 1 edge.

1. **`remove_edge` removes edge** — Start with 2 edges, apply `remove_edge` for one, verify 1 remains.

1. **`update_node_data` merges data** — Node with `{ label: "Workers" }`, apply update `to: { label: "D1" }`, verify label changed.

1. **`update_node_data` preserves other data fields** — Verify updating `label` does not remove `description`.

1. **`update_edge_data` merges data** — Edge with `{ label: "HTTP" }`, apply update `to: { protocol: "HTTPS" }`, verify protocol added.

1. **`update_edge_data` preserves other data fields** — Verify updating `protocol` does not remove `label`.

1. **`batch` applies all sub-operations in order** — Create a batch that adds a node then adds an edge. Verify both are present.

### Round-Trip Tests

1. **Round-trip for every operation type** — For each of the 8 operation types: start with known state, apply the operation, apply its reverse, verify state matches original. This is a parameterized/table-driven test covering `add_node`, `remove_node`, `move_node`, `add_edge`, `remove_edge`, `update_node_data`, `update_edge_data`, and `batch`.
